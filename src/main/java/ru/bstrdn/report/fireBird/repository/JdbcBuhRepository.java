...
public class JdbcBuhRepository {
    ...
    /**
     * 1 бухгалтерский отчет - по сертификатам
     *
    ...
    public List<Report_buh_1> queryReport_buh_1(Integer certId, String startDate, String endDate) {
        ...
        stringBuilder.append("""
                ...
                GROUP BY cl.fullname, sert.sumrub, n_saldo, dates_pay, r_amountrub, k_saldo, cer.cname
...
//        if (!report_buh_1.isEmpty()) {
//            int summ = report_buh_1.stream()
//                    .mapToInt(Report_buh_1::getSumm)
//                    .sum();
//            Report_buh_1.setSum(summ);
//        }
//        report_1.forEach(r -> r.setDocFullname(r.getDocFullname().replaceAll(FIO_PATTERN, "$1$2. $3.")));
    ...
    /**
     * 3 бухгалтерский отчет - По сертификатам
     */
    public List<Report_buh_3> queryReport_buh_3(Integer certId, String startDate, String endDate) {
        ...
        List<Report_buh_3> report_buh_3 = jdbcTemplate.query(stringBuilder.toString(), ROW_MAPPER_BUH_3, startDate, endDate, startDate, endDate);
//        report_1.forEach(r -> r.setDocFullname(r.getDocFullname().replaceAll(FIO_PATTERN, "$1$2. $3.")));
        return report_buh_3;
    }
    ...
    /**
     * бухгалтерский отчет - Акт сверки
     */
    ...
    /**
     * Таблица с информацией для для заполнения АКТА СВЕРКИ
     */
...
//        Timestamp end = Timestamp.valueOf(endDate);
        return jdbcTemplate.query("""
                ...
                COALESCE (start_per1.sum_uslug, 0)org1_oborot,
                ...
                WHERE pers.jid = ? --2 Фильтр по юр лицам
                AND t.amountjp_disc != 0
                AND t.treatdate < ?   --Фильтр дата (Подставляем начало периода)
                ...
                AND jp.jid = 10000117       --2Фильтр по Юр. лицам
                ...
                JOIN jpersons pers ON dog.jid = pers.jid
                JOIN jpersons pers_clinic ON t.clinicid = pers_clinic.jid
                WHERE pers.jid = 10000150 --2 Фильтр по юр лицам
                AND t.amountjp_disc != 0
                AND t.treatdate >= ? AND t.treatdate <= '31.01.2021'  --Фильтр дата (Подставляем выбранный период)
                ...
                FROM  jaccpay jp
                LEFT JOIN jpersons pers ON jp.jid = pers.jid
                WHERE jp.pdate >= ? AND jp.pdate <= '31.01.2021'    --Фильтр дата (Подставляем выбранный период)
                AND jp.jid = 10000117       --2Фильтр по Юр. лицам
                GROUP BY org2)end_per2 ON start_per2.org2 = end_per2.org2
                """, ROW_MAPPER_AKT_SVERKI_INFO, orgId, start).get(0);
    }
}
